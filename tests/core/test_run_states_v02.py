...
def test_step_attempt_is_frozen_and_hashes_input_deterministically() -> None:
    """step attempt is frozen and hashes input deterministically."""
    first = StepAttempt(
        run_id="run-1",
        step_id="quant",
        attempt_no=1,
...
def test_step_attempt_appends_state_event_without_consuming_retry() -> None:
    """step attempt appends state event without consuming retry."""
    running = StepAttempt(
        run_id="run-1",
        step_id="news",
        attempt_no=2,
...
def test_waiting_states_are_not_terminal() -> None:
    """waiting states are not terminal."""
...
def test_run_state_values_match_contract() -> None:
    """run state values match contract."""
...
def test_step_attempt_rejects_invalid_sequence_numbers(
    ...
    """step attempt rejects invalid sequence numbers."""
    with pytest.raises(ValidationError):
        StepAttempt(
            run_id="run-1",
            step_id="quant",
