...
- v0.3 `11-valuation_discovery` 已接入数据层公司池快照，排除 ST、`退市*`、未来上市和已退市证券；新增第四层 Quant Feature Mart `quant_feature_snapshots`/`quant_feature_rows`，第三层 ETL 事务性发布后供量化只读；量化结果再事务性发布为 Analysis Mart `analysis_snapshots`/`analysis_metrics`/`analysis_findings`/`analysis_evidence_links`，供 AI/Dashboard 读取；最新真实量化 run `qr_df48cd92fdf1424d` 基于 `cps_29518c0fec90836c57609b6f1f24` 产出 5304 家结果、3 家 pass、54 家 near_threshold。
- v0.3 `06-multi_agent_research` 已开放第四层 Mart scoped read tools：`analysis_snapshot_get`、`analysis_metrics_list`、`analysis_findings_list`、`quant_feature_snapshot_get`、`quant_feature_rows_list`，`valuation_analysis` 节点可按 security/scope/PIT 读取第四层特征、分析快照、指标和发现。
