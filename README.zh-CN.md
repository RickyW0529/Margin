...
Margin v0.3 已经打通 Tushare 量化数据链路和研究候选闭环：

- Tushare 独立源系统表、源质量筛选和统一仓库发布；
- 量化需求 endpoint 准入，排除没有活跃消费方的数据；
- 近 24 个月滚动行情/复权、财务三表/指标、估值快照、停牌事实和 benchmark 数据；
- 非 ST、非退市、非未来上市的全 A 公司池快照进入 QuantInput；
...
- 研究工作台前端：候选快照、已实现入口导航、服务端筛选、current-vs-effective 结论、证据 locator、只读 Copilot、Provider blocker 展示和 Provider 设置；
...
- `/`：研究工作台总览、候选快照、推荐操作顺序和 Provider 状态；
- `/research`：候选列表、筛选、刷新触发、Provider blocker 和只读 Copilot；
- `/settings/data`：滚动数据采集策略配置；
- `/settings/providers`：只写 Provider 密钥与 health 激活。
...
| 当前设计文档索引 | [docs/design/v0.3/README.md](./docs/design/v0.3/README.md) |
