...
> 文档版本：v0.2
> 状态：draft
> 当前实现：当前代码仍为 v0.1 基线；本文件描述 v0.2 增量目标
...
v0.2 不再把“输入一个证券代码并立即研究”作为主要入口，而是持续维护一个可选择公司池的内在价值研究账本。第一期只实现动态沪深 300；后续公司池可扩展为中证 500、沪深 300 + 中证 500、行业池和全 A。

### 0.1 产品目标

- 回答“这家公司基于当前可用证据本应该值多少”，不预测明日价格；
- 沪深 300 全部公司进入量化层并全部展示，不隐藏被淘汰公司；
...
| 更新类型 | 处理 |
| --- | --- |
| 首次初始化 | 回填最近 5 年财务与估值历史、最近 3 年行情、最近 1 年公告、最近 90 天新闻；重大事项按需追溯 3 年 |
| 每日同步 | 增量同步价格、财务披露、公告、新闻和公司池变化 |
| 仅价格变化 | 重算当前折价率、安全边际和低估状态；不重新调用 AI |
| 新财报/业绩预告 | 重新计算量化指标并触发 AI 研究 |
| 重大公告/实质新闻 | 写入快照与向量库，触发受影响公司的 AI 更新 |
...
量化闸门至少覆盖：估值历史分位、行业相对估值、盈利能力、ROE/ROIC、经营现金流、自由现金流、负债、增长质量、财务异常和数据完整度。行业模型由系统管理，银行、保险、周期、消费/制造和成长行业不能使用同一套 PE 规则。
...
1. Provider：数据、WebSearch、LLM、Embedding/Rerank 的选择、密钥和优先级；
2. 公司池：第一期沪深 300，后续扩展其他指数和行业池；
3. 量化闸门：系统默认阈值、用户调整、恢复默认、版本化；
4. AI 投资风格 Prompt：价值、成长、股息、逆向等投资理念和用户关注重点。

系统 Guardrail Prompt、证据引用要求、时点约束、结构化输出 Schema、行业估值公式、Agent 编排和工具权限不可由普通用户覆盖。Prompt 修改必须生成新的策略版本。
...
v0.1 已经形成一条可运行的研究闭环，v0.2 在此基础上增加公司池与估值发现能力：
...
v0.2 不面向高频交易、自动下单、融资融券、券商账户托管、投顾业务或多租户 SaaS。
...
v0.2 的通知是本地结构化 alert，不接短信、邮件或 IM。
...
| P-10 | v0.2 设计增量可审计，开发过程 spec/plan 按功能模块拆分 | `design/v0.2` + `AGENTS.md` 规则检查 |
...
- 全公司量化闸门及可解释淘汰原因；
- 行业专属估值模型和内在价值区间；
- 基于新财报、公告、新闻和行业变化的事件驱动 AI 更新；
- 全公司研究状态与低估置信度面板；
- Provider、公司池、量化阈值和投资风格 Prompt 配置；
