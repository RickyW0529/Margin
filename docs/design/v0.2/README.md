...
本目录是 Margin v0.2 的产品与架构设计快照。它由 `design/v0.1/` 直接复制后增量迭代：保留 v0.1 已实现架构作为基线，在 v0.2 中把产品主线升级为“公司池 + 多因子量化筛选 + 量化目标驱动 NewsRefresh + 行业估值 + AI 增量复核 + 全公司估值发现面板”。
...
v0.2 的核心变化是把产品入口从“用户输入证券代码后研究”改为“系统动态加载公司池并持续维护内在价值判断”。底层数据仓库全量采集已启用 Provider 可提供的全 A 结构化数据，用户可选择沪深 300、中证 500 或全 A 及相应指标集；作用域内全部公司都展示，先经过确定性量化闸门，再对量化通过、接近阈值、状态变化、复核到期或价格触发观察区间的公司补充相关新闻，并让 AI 结合上一版结论、当前量化结果和当日重要新闻执行增量复核。产品目标不是预测明天股价，也不是分析用户现有持仓，而是回答“基于当前证据，这家公司合理应该值多少钱，以及今天是否需要改变原判断”。
...
| 首期公司池 | 沪深 300、中证 500 和全 A；公司池定义可配置，成员使用双时态拉链并保留生成快照 |
| 数据层 | 所有启用 Provider endpoint 按全证券范围采集全部返回字段；完整 Raw Snapshot、多源标准事实和 Canonical 推荐值分层保存 |
| 量化层 | Phase 1 为单日截面多因子筛选；`valuation_discovery/quant` 通过 `QuantDataAdapter` 读取 canonical PIT 数据，先硬过滤，再计算 Quality/Value/Growth/Momentum/Risk 分、状态、guardrail 和原因 |
| News 层 | 不做无边界全网全量采集；`NewsRefreshService` 只接收量化目标公司，统一搜索、抓取、快照、公司关联、重要度过滤、分块和向量入库 |
| AI 层 | 基于上一版 AI 结论、当前量化结果、重要新闻和 RAG 证据做增量复核；AI 不直接拉外部行情/财务接口，也不在推理阶段临时 WebSearch |
| 编排层 | APScheduler 只负责触发；`ValuationDiscoveryOrchestrator` 通过 `valuation_refresh_runs/steps` 串联后端流程；LangGraph 只用于 AI 内部图 |
| 更新模式 | 每日自动增量同步结构化数据；启动时检查 freshness，已最新则跳过，过期则后台同步；量化完成后按完整目标集补新闻；AI 输出 `CARRY_FORWARD_VERIFIED`、`UPDATE_ASSESSMENT`、`DOWNGRADE_CONFIDENCE`、`INVALIDATE`、`ABSTAIN` 或 `REVIEW_DEFERRED` |
| 估值输出 | 内在价值区间、估值置信区间、低估置信度、价值陷阱风险、观察价格区间、研究周期、失效条件 |
| 用户配置 | Provider、公司池、指标集、量化闸门、AI 投资风格 Prompt |
| 系统管理 | 证据护栏、结构化输出 Schema、行业估值模型、Agent 编排、工具权限 |
| 新模块 | `11-valuation_discovery`：公司池快照、`valuation_discovery/quant` 多因子量化筛选、行业估值模型、估值快照和刷新事件 |
...
| 回测范围 | 回测、绩效归因和报告导出属于 Phase 2，不阻塞 v0.2 主链路的单日截面筛选 |
...
| 工具扩展 | v0.2 使用内部工具定义目录、ScopedToolFactory、ToolPolicyEngine 和 ToolExecutor，不做 MCP Server 或用户自定义工具 |
| 持仓分析 | v0.2 不实现；历史源码、API、页面、调度和数据库表已删除 |
...
v0.2 已删除 v0.1 的组合、持仓和持仓监控实现，包括源码、API、页面、Worker 调度与数据库表。模块编号 02/09 仅用于历史审计。
...
| ER 图 | 架构设计 | 展示 v0.2 目标仓库关系和 v0.1 历史迁移审计表关系 |
...
工具能力统一通过内部工具定义目录、`ScopedToolFactory`、`ToolPolicyEngine`、`ToolExecutor`、类型化 Provider Adapter、固定权限等级和审计记录接入。后续 v0.3 继续从本目录复制并增量迭代，不回写 v0.2 的审计边界。
...
v0.2 支持前端只写录入 Provider 密钥并保存到本地加密 Secret Store。环境变量只用于 Secret Store 主密钥、首次迁移和无 UI 运维预置；任何真实密钥都不能提交到 Git、写入文档或出现在日志/测试输出。
...
- 2026-06-20：补充 v0.2 设计决策和后续按功能模块拆 Superpowers 临时 spec/plan 的边界。
