...
> Boundary: the codebase still implements the v0.1 baseline; v0.2 does not implement MCP Server, MCP Gateway, arbitrary custom HTTP tools, or broker order execution.

---

## 0. v0.2 Architecture Increment

v0.2 adds module `11-valuation_discovery` to the existing modular monolith. It owns universe snapshots, industry-aware valuation models, the quantitative gate, intrinsic-value assessments, confidence calibration, and research-refresh events. Existing modules remain responsible for providers, filings, vector indexing, evidence, AI workflows, strategy versions, dashboards, and runtime scheduling.

```mermaid
flowchart LR
    Data[01 Data Providers] --> Universe[Universe snapshots]
    Data --> Fundamentals[Point-in-time fundamentals]
    Universe --> Quant[11 Quant gate]
    Fundamentals --> Quant
    Quant --> Rejected[Visible rejection result]
    Quant --> Passed[Passed company]
    Filings[03 Filing/news delta] --> Index[04 pgvector indexing]
    Index --> Evidence[05 Evidence]
    Evidence --> AI[06 AI research]
    Passed --> AI
    Strategy[07 Versioned universe/gate/style prompt] --> Quant
    Strategy --> AI
...
- `UniverseProvider` and `UniverseSnapshotService`;
...
Suggested persistence entities are `universe_definitions`, `universe_snapshots`, `universe_members`, `fundamental_snapshots`, `quant_screen_runs`, `quant_screen_results`, `research_refresh_events`, `valuation_assessments`, and `valuation_evidence`.

Daily jobs synchronize changed prices, financial disclosures, filings, and news. Price-only changes reprice existing assessments. AI consumes only material refresh events or scheduled review expiry. Source content and embeddings are updated idempotently by content hash.
...
v0.2 uses typed adapters and an internal tool registry instead of MCP.
...
The v0.2 database has 29 public tables. Current positions are calculated from trades and are not stored in a separate `positions` table.
...
Margin v0.2 connects portfolio data, evidence retrieval, AI research, dashboard publication, holdings monitoring, audit, and local deployment. Its core architectural property is conservative auditability: when evidence or providers are weak, the system abstains rather than creating false certainty.
