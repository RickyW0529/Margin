...
> 上一版本基线：v0.2 active
> 重要边界：当前代码仍以 v0.2 通用 Raw/Fact/Canonical 数据路径为基线；v0.3 重构数据分层，不实现自动下单、持仓分析或持仓监控。
...
flowchart TB
    ...
    S --> Q[2. 质量筛选层<br/>Schema/完整性/重复/PIT/异常/冲突/发布]
    Q --> W[(3. 统一数据仓库层<br/>维度/事实/Canonical/PIT)]
    W --> U[4. 公司池视图层<br/>ALL_A_NON_ST/CSI300/CSI500/自定义快照]
    U --> A[5. 上层服务<br/>QuantInput/量化/估值/News/AI/Dashboard]
```

禁止依赖：

- 数据仓库层不得直接调用 Provider，只能消费质量层发布批次；
- 公司池视图层不得读取 `source_*` schema；
...
公司池视图使用仓库 PIT 数据生成不可变快照。`ALL_A_NON_ST` 排除 `ST`、`*ST`、
退市整理、已退市和非普通 A 股；每个排除项保存 reason、源状态、`business_at`、
`known_at` 和仓库版本。公司池视图是 QuantInput 的证券范围入口。

#### 0.1.5 上层服务

上层只通过类型化 repository/service 使用仓库和公司池。QuantInput 冻结公司池成员、
canonical fact IDs、市场窗口、质量/PIT 状态和输入 hash；量化为每个成员保存通过、
淘汰或数据不足结果，验收输出具体公司、分数、排名和结构化原因。
...
#### 0.1.7 QuantDataRequirementCatalog
