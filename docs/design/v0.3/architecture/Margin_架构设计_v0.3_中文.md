...
#### 0.1.5 第四层 Mart 与 ETL 管道

第四层 Mart 是 v0.3 面向量化、AI 流和 Dashboard 的服务表，不替代底层 canonical
facts，也不作为人工可编辑结果表。ETL 管道集中由 valuation discovery ETL 层管理，
不得分散在上层节点里直接写表。

第三层唯一化数据先物化为量化可直接读取的 Quant Feature Mart：

- `quant_feature_snapshots`：按 scope、universe、decision time、trading date 保存一次
  第三层 ETL 的特征快照、输入 hash、特征列、lineage summary、质量标记和行数；
- `quant_feature_rows`：按 security 保存量化可直接读取的估值、财务、行情、行业、
  ST/停牌等特征和 source refs。

量化层只读取 `feature_snapshot_id` 对应的第四层特征截面，不再在运行时从第三层临时
拼接全量截面。量化结果和可校验证据引用再物化为 Analysis Mart，按
security/scope/decision time 保存：
...
事务边界为 ETL 层硬约束：第三层到 Quant Feature Mart 的发布必须在一个事务内写入
绑定 `feature_snapshot_id` 的 `QuantInputSnapshot`、fact lineage、feature snapshot 和
feature rows；量化结果到 Analysis Mart 的发布必须在一个事务内写入 snapshot、metrics、
findings 和 links。任一 child row 冲突或写入失败必须回滚本次 ETL，不允许留下只有
header 或部分 rows 的脏数据。
