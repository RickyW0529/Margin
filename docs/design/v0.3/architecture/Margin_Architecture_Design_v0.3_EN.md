# Margin Open Investment Research System | Architecture Design v0.3
...
```mermaid
flowchart TB
    subgraph S[1. Independent source systems]
        TS[(source_tushare<br/>endpoint-specific tables)]
...
plus stable lineage, natural-key hash, payload hash, business/PIT timestamps, schema version,
and revision number.

The quality boundary stores versioned rulesets, reports, issues, quarantine records, and
publication batches. Decisions are `publish`, `publish_degraded`, `quarantine`, or
`mapping_required`; no decision deletes source data.

The warehouse stores unified security/status history, industry/concept/index memberships,
market and adjustment data, financial statements and indicators, valuation, corporate
actions, multi-source facts, and canonical values. Every row links to a quality publication
and source record.

`ALL_A_NON_ST` excludes ST, `*ST`, delisting-transition, delisted, and non-common-A-share
securities with PIT evidence and exclusion reasons. QuantInput freezes this universe plus
...
Append-only `data_acquisition_policy_versions` stores a local-admin-owned policy with
`rolling_window_months` (default 24, server range 12–60),
`revision_lookback_days`, `financial_comparison_years`, lifecycle timestamps, and a
stable config hash. Every sync run freezes the policy version and calendar-derived
`window_start`/`window_end`.

Date-series partitions may not exceed the frozen window. Security master, name/ST
status, listing/delisting, corporate actions, lineage, and audit history are retained
as required for PIT correctness. Fundamentals may load one minimal comparison year
before the serving window and mark those rows `comparison_only`.

The frontend exposes `/settings/data` with the active policy, resulting date range,
coverage state, a validated 12–60 month input, and separate actions for creating a
version, activating it, and starting a background sync. Writes require local-admin
authorization, CSRF, idempotency, and append-only audit.

#### 0.1.2 Quant-driven endpoint admission

`QuantDataRequirementCatalog` computes the transitive closure from active quant features,
hard-filter inputs, `ALL_A_NON_ST`, PIT/adjustment, and industry/index benchmarks to
warehouse datasets and provider endpoints. Only endpoints linked to at least one active
requirement may create sync partitions. The current risk factor admits aggregate pledge
statistics only; pledge-detail rows remain excluded. Management, shareholder-list,
top-list, block-trade, margin-financing, concept-theme, and popularity datasets remain
out of scope until a versioned quant formula and warehouse consumer explicitly require them.

#### 0.1.3 Index and query-plan acceptance

Indexes follow measured access paths: source natural-key/revision uniqueness, symbol/date
and endpoint/date scans, partial indexes for pending partitions and publication queues,
warehouse security/indicator/event/PIT reads, universe snapshot membership, and quant run
ranking. Large payload JSONB is not given a blanket GIN index.

After realistic backfill, `EXPLAIN (ANALYZE, BUFFERS)` must validate partition pruning
and expected index usage for universe materialization, QuantInput cross-sections, market
history, partition claiming, and quality publication. Duplicate, unused, low-selectivity,
and bloated indexes are audited.

### 0.2 v0.2 baseline flow constrained by section 0.1
...
The v0.1 baseline is a working local research system; v0.3 adds continuous universe-level valuation discovery.
...
v0.3 uses typed adapters and a scoped internal tool system instead of MCP.
...
- old `research_candidate/watch` signals are not mapped directly to v0.3 delta-decision outcomes.
...
`universe_definitions` stores stable data-driven identities rather than a database enum. `universe_definition_versions` stores names, rule code/configuration, lifecycle, and valid/system ranges. v0.3 seeds `CSI300`, `CSI500`, and `ALL_A`. `universe_snapshots` records each complete materialization run, while `universe_memberships` stores `universe_id`, `security_id`, valid/system ranges, weight/rank, inclusion reason, the materialized universe snapshot, optional raw source snapshot/run, hash, and quality.
...
### 11.1 v0.3 warehouse relationships
...
The ER graph below is retained as a v0.1 migration-audit artifact. Migration `20260623_0031_remove_holdings` removes `portfolios`, `trades`, `position_theses`, `alert_events`, and `position_reviews`; those entities are not part of the current v0.3 runtime schema.
...
- `MARGIN_DATA_SYNC_STARTUP_MODE`, fixed to `background_non_blocking` in v0.3;
- `MARGIN_MONITORING_INTERVAL_SECONDS` for retained v0.1 worker behavior; v0.3 valuation refresh should use a separate refresh interval setting.
...
v0.3-specific verification adds universe snapshot tests, data warehouse tests, quant-gate tests, refresh-policy tests, valuation-calibration tests, evidence-grounded AI output tests, and all-company dashboard E2E coverage.

## 16. Summary

Margin v0.3 connects provider sync, a point-in-time data warehouse, quant gates, ResearchContext, evidence retrieval, AI research, valuation snapshots, dashboard publication, audit, and local deployment. Its core architectural property is conservative auditability: when data, evidence, or providers are weak, the system abstains rather than creating false certainty.
