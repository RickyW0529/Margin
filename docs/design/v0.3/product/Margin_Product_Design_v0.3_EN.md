# Margin Open Investment Research System | Product Design v0.3
...
v0.3 keeps the v0.2 valuation-discovery product direction and changes only the structured-data foundation. Acceptance is centered on broad Tushare coverage, a non-ST All-A universe, and real company-level quant output.

```mermaid
flowchart LR
    TS[(Independent Tushare source tables)]
    AK[(Independent AKShare source tables)]
    O[(Future independent source tables)]
    TS --> Q[Data quality gate]
    AK --> Q
    O --> Q
    Q --> W[(Unified warehouse)]
    W --> U[Universe view layer]
    U --> S[Quant / valuation / news / AI / dashboard services]
```

Each provider owns an isolated endpoint catalog, sync runs, partitions, call audit,
raw snapshots, and endpoint-specific landing tables. The quality layer is the only
path into the warehouse. Universe views read only the warehouse, and upstream services
may not query provider landing tables or call providers directly. Tushare stock endpoints
are probed only when they are in the transitive closure of active quant features, hard
filters, universe rules, PIT/adjustment, or benchmarks. The project targets roughly 95%
engineering confidence that quant-required APIs are substantially exhausted. Endpoints
without a quant consumer are cataloged as out of scope and are not backfilled. Acceptance
prints real companies with names, scores, ranks, statuses, and structured reasons.

### 0.1.1 Rolling window and frontend configuration

- A versioned `DataAcquisitionPolicy` defaults to a 24-month rolling window.
- The frontend Data Policy page lets a local administrator create and activate a
  12–60 month version and previews the resulting start date.
- Activation affects newly created background sync runs; it never performs a long
  backfill in the request thread.
- Date-series data is windowed. Fundamentals retain only the minimal earlier
  comparison periods required for TTM, year-over-year, and consecutive-year rules.
- Security master, name/ST status, listing/delisting, corporate actions, lineage,
  and audit history are exempt from hard window deletion where PIT correctness needs them.
- Reference-aware retention may remove expired unreferenced facts asynchronously.
...
- The system estimates what a company should be worth; it does not predict tomorrow’s price and does not analyze the user’s current holdings in v0.3.
...
v0.3 should feel like a valuation-discovery workspace, not a symbol lookup form:
...
Module IDs 02 and 09 are retained only for historical audit. Their implementations are removed in v0.3.

## 1. Product Summary

The v0.1 baseline turns a scattered personal investment workflow into an auditable loop; v0.3 shifts the product line to continuous universe-level valuation discovery:
...
v0.3 does not implement holdings alerts, email/SMS/IM notifications, or position reviews. It only exposes research refresh status inside the universe dashboard and company detail pages.
...
- strategy configuration has backend APIs in v0.1; the v0.3 frontend must expose provider, universe, indicator-set, gate, and style-prompt settings;
...
- `risk_review` and `reflect_counter_argument` are structured in v0.1; v0.3 requires per-item evidence references;
- quant backtesting, performance attribution, and report export are Phase 2; they do not block the v0.3 single-day cross-sectional screen;
...
v0.3 should add evidence-grounded risk/counter-argument generation, including per-item `evidence_ids`, locators, stricter language/output controls, provider configuration UI, universe and indicator-set configuration, Phase 1 multi-factor quantitative screening under `valuation_discovery/quant`, and investment-style prompts.

## 11. Summary

The v0.1 baseline delivers a working local research loop. v0.3 is a draft design that extends it into continuous intrinsic-value discovery; the new universe, quant-gate, targeted news refresh, industry-valuation, and AI delta-review capabilities are not implemented yet.
