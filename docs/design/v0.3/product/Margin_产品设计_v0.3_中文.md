# Margin（安全边际）开源投资研究系统｜产品设计文档 v0.3
...
> 状态：review
> 上一版本基线：v0.2 active
> 当前实现：v0.3 数据主链路、真实量化结果和 Analysis Mart 第四层已实现；AKShare 实网回填按降级边界处理
...
v0.3 不改变 v0.2 的估值发现产品方向，唯一主增量是重构结构化数据底座，并把验收收敛为尽可能完整的 Tushare 数据、非 ST 全 A 公司池和真实量化公司结果。
...
flowchart LR
    ...
    Q --> W[(统一数据仓库层)]
    W --> U[公司池视图层]
    U --> S[量化/估值/News/AI/Dashboard 服务]
```

每个数据源拥有自己的接口目录、同步任务、调用审计、原始快照和 endpoint 专用
landing 表。质量筛选层是进入统一仓库的唯一入口；公司池只读统一仓库；上层服务
只读公司池视图和仓库服务契约，禁止直接调用 Provider 或读取源系统表。
...
- Tushare、AKShare 和未来数据源不能共用一组源数据业务表；每个 endpoint 使用独立 landing 表保存完整返回行和修订历史；
- 通过候选接口目录、官方/SDK 资料和当前席位真实探测交叉验证，在约 95% 工程置信度下确认量化所需可用接口已基本穷尽；
- 每个 endpoint 必须标明消费它的量化特征、硬过滤规则、公司池、PIT/复权或 benchmark；没有需求引用的接口不进入采集计划；
- 数据只有通过质量筛选层发布判定后才能进入统一仓库；被隔离或未映射数据仍保留在源系统层；
- 在线结构化数据默认使用最近 24 个月滚动窗口，用户可在前端配置 12–60 个月；每日同步按当前日期推进窗口，不形成无限历史堆积；
- AKShare/Tushare 的原始响应、多源标准事实和字段生命周期完整保留；Canonical 层只选择推荐值，不覆盖其他来源；
- PostgreSQL 中的时点数据仓库是量化和 AI 的唯一事实来源；
- `ALL_A_NON_ST` 公司池从仓库证券状态/名称变更构建，排除 ST、*ST、退市整理和非普通 A 股并保存排除原因；
...
- v0.3 不做用户持仓分析、持仓监控、买入后告警或持仓复盘；
- 每次量化结果、AI 结论、估值区间和触发原因均保存为不可变快照。
- 验收必须输出真实量化运行中的具体公司代码与名称、总分、五组分数、排名、过滤/复核原因和数据状态。

### 0.1.2 量化需求驱动的采集准入

系统维护版本化 `QuantDataRequirementCatalog`，从 active QuantFeatureSet、硬过滤规则、
公司池规则、PIT/复权和 benchmark 需求计算 endpoint 闭包。当前准入范围：

- 证券主数据、名称/ST 状态、上市/退市和交易日历；
- 日行情、成交额、复权因子、停复牌、每日估值指标；
- 利润表、资产负债表、现金流量表、财务指标、业绩预告/快报、审计意见；
- 风险因子实际消费的聚合质押比例；不采集逐笔质押明细；
- 计算复权与 PIT 所必需的公司行动；
- 行业分类和指数行情/权重，用于行业标准化和 benchmark；
- 由 active 量化策略新增并具有明确公式/消费字段的其他数据。

默认不采集当前量化未消费的管理层、股东名单/人数、质押明细、龙虎榜、大宗交易、
融资融券、概念题材、新闻热度等数据。目录可记录这些 endpoint 为 `out_of_scope`，
但不得创建回填分片或 landing 数据。

### 0.1.1 滚动窗口与前端配置

- 新增版本化 `DataAcquisitionPolicy`，默认 `rolling_window_months=24`；
- 前端“数据策略”页允许本地管理员创建 12–60 个月的新版本、查看预计窗口起点并激活；
- 激活不在请求线程执行回填，只影响之后创建的数据同步 run；用户可另行触发后台同步；
- 行情、估值、资金流等日期序列按窗口裁剪；
- 财务数据按报告期/披露期覆盖窗口，并额外保留计算 TTM、同比和连续年度规则所需的最少前置比较期；
- 证券主数据、名称/ST 状态、上市/退市、公司行动和源审计不按 24 个月硬删除，避免 PIT 与公司池失真；
- 窗口缩短时先停止继续服务过期事实，再由引用感知 retention 异步清理；仍被 QuantInput、研究快照或审计引用的数据不得删除。
...
v0.3 的数据链路必须先全量入库再按作用域消费。量化层和 AI 不直接调用 AKShare、Tushare 或其他外部行情/财务接口，避免外部接口数据变动导致研究结果不可复现。
...
v0.3 的默认用户路径应从“先想起某只股票”改成“先看系统发现了什么”：
...
Research Board Copilot 在 v0.3 只承担只读信息整合：理解“今天哪些公司值得继续看”“为什么结论没变”等意图，调用与当前页面和作用域绑定的查询、筛选、聚合和证据导航能力。它不启动另一套自主研究流程，不越过 `AIDeltaReviewGraph` 生成新估值，不调用实时 WebSearch，也不修改策略、密钥或运行状态。
...
跨模块能力通过接口和事件串联，不能把 v0.3 写成一个大模块。
02 持仓与 09 持仓监控仅保留历史编号；v0.3 已删除其实现，不进入产品主线或验收。
...
v0.1 已经形成一条可运行的研究闭环，v0.3 在此基础上把主线切换为公司池与估值发现：
...
| 底层全量采集 | 用户作用域不裁剪采集任务；所有启用接口按全证券范围增量同步并回抓修订窗口 | v0.3 provider endpoint scheduler |
| 数据先入库 | Provider 采集结果必须先形成完整 raw snapshot、多源标准事实、质量结果和 canonical 选择，再被量化或 AI 消费 | v0.3 data warehouse |
...
| 全量可见 | 公司池内全部公司都有状态、原因和最近更新时间 | v0.3 全公司估值发现面板 |
...
v0.3 不面向高频交易、自动下单、融资融券、券商账户托管、投顾业务、多租户 SaaS，也不做用户持仓分析或持仓监控。
...
12. AI 研究前构造 `ResearchContext`，明确引用 `QuantInputSnapshot`、当前量化结果、Analysis Mart 快照、上一版有效 AI 结论、用户指标视图和重要新闻/RAG 证据，不由 LLM 现场拉取外部行情/财务数据；
...
15. 量化结果和可复用数据分析结果落入 Analysis Mart 第四层，AI scoped tools 与 Dashboard 可以读取 snapshot、metrics、findings 和证据链；
16. 公司详情页展示最新快照、历史快照差异、新闻复核状态、编排步骤状态和对应数据快照版本。

## 6. 页面与信息架构

当前前端使用 Next.js App Router，v0.3 页面主线聚焦公司池估值发现。组合/持仓页面已删除，不进入主导航和验收路径。
...
v0.3 的策略页面尚未完全产品化为前端配置中心，但后端能力已可供后续 UI 接入。
...
- `RiskReviewAgent` 与 `ReflectCounterArgumentAgent` 在 v0.3 中返回结构化风险、反方理由和未知项，并记录 `model_version` / trace；它们不保证每条风险或反方理由都有独立证据引用。逐条证据约束、中文输出约束和更严格 evidence-grounded prompt 属于 v0.3。
- `ResearchSignalComposer` 正常路径优先调用 LLM 生成结构化 signal；行情退化、引用校验失败或 LLM 失败时，走规则型保守输出。

## 9. 刷新状态与复核提示设计

v0.3 不做持仓告警，也不接短信、邮件或 IM。系统只在公司池和公司详情内展示研究刷新状态，帮助用户判断某家公司是否需要重新阅读。
...
v0.3 的验收不是 51 项同时开始、同时完成，而是三个连续交付门：
...
| P-15 | AI 研究必须基于后置 `ResearchContext`，该快照显式引用本轮 `QuantInputSnapshot`、量化结果、Analysis Mart 快照、NewsContextBundle、上一版有效结论、RAG 证据和受控工具结果 | workflow contract test |
...
| P-38 | `quant_screen_runs` 与 `quant_screen_results` 必须保存策略版本、数据版本、因子明细、缺失字段、风险标记、排名和可读原因摘要，并能发布到 Analysis Mart | repository contract test |
...
| P-43 | 图内工具限定为冻结上下文、Analysis Mart、量化结果、财务快照、估值、RAG、已存 WebSearch 快照、受限计算和引用校验等只读工具 | scoped tool policy test |
...
| P-61 | Analysis Mart 第四层保存 snapshot、metrics、findings 和 evidence links；同一输入重放幂等，冲突重放拒绝 | analysis mart repository tests |
| P-62 | AI scoped tools 可按 security/scope/PIT 读取 Analysis Mart snapshot、metrics、findings，跨证券或未来快照返回空结果 | analysis tool policy tests |
...
- v0.3 P2 必须完成 Provider、公司池、用户指标视图、量化阈值和投资风格 Prompt 的前端配置中心；
- Rerank 是可选 Provider；
- Provider 密钥前端配置属于 v0.3 P2；环境变量只用于 Secret Store 主密钥和无 UI 的运维预置；
- 风险与反方审查当前是结构化 LLM 输出，但不强制逐条证据引用；
- 量化回测、绩效归因和报告导出属于 Phase 2，不阻塞 v0.3 主链路的单日截面筛选；
- 大规模历史回测和 Parquet/DuckDB 分析层仍未作为 v0.3 生产主链路。
...
v0.3 设计评审通过后，使用 Superpowers 在被 Git 忽略的 `docs/superpowers/` 下按功能模块分别生成 spec 与详细 plan，禁止把整个版本合并成一份文档。v0.3 应从 `docs/design/v0.3` 复制后继续增量迭代。

## 13. 总结

Margin v0.1 已经形成可运行的本地研究闭环；v0.3 设计把它升级为持续维护公司内在价值的研究系统。当前 v0.3 尚处于设计阶段，不应把本文中的增量能力视为已实现。
