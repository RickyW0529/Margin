...
持续验证当前持仓的投资逻辑是否仍成立、组合风险是否需要复核，并在盘中与盘后按规则触发分级提醒。盘中只做确定性规则检测、证据检索与轻量 AI 解释，不执行重新训练、全市场研究、任意 Agent 长链、自动下单或无规则限制的自由研究结论。
...
盘中监控流程（架构 §19）：Price Poller + News Poller → 规则引擎 → 触发事件? → 证据检索 → 轻量 AI 解释 → 逻辑改变? → P0/P1 提醒（逻辑改变）或 P2/P3 提醒（未改变）。
