...
- `11-valuation_discovery` now consumes data-layer company-pool snapshots and publishes the fourth-layer Quant Feature Mart + Analysis Mart. Third-layer ETL transactionally publishes `quant_feature_snapshots/rows` for quant-only reads, and quant output is then transactionally published as `analysis_snapshots`, metrics, findings, and lineage. The latest full-A real quant run `qr_df48cd92fdf1424d` used 5304 non-ST/non-delisting/non-future-listed companies and produced 3 pass and 54 near-threshold names; the 2026-07-02 CSI300 end-to-end refresh `vdr_abcdbd61a870a9e1e8024343` used 300 real inputs and produced one near-threshold name.
