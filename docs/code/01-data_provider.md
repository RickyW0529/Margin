...
| 财务 | `income`、`balancesheet`、`cashflow`、`fina_indicator`、`fina_audit` 覆盖 2020-12-31 至 2026-03-31 的财务窗口；`income` 只发布原始 `n_income_attr_p`，不在 Provider 层派生 `net_profit_ttm` 或年度净利。 |
...
| `get_financials` | `def get_financials(self, symbols, start, end) -> list[dict[str, Any]]` | 返回财务指标：`report_date`、`ann_date`、`roe`、`eps`、`gross_profit_margin`，并用 `income.n_income_attr_p` 对齐同一报告期发布原始归母净利。 |
