...
`08-research_candidate_dashboard` 是 Margin 当前实现 的研究候选面板模块，负责把模块 06（`margin.research`）产出的工作流结果转换为可人工审阅的候选卡片、证据展开、估值视图与研究报告，并通过 FastAPI + Next.js 页面暴露给用户。
...
| `JobStatus` | `completed`、`failed` | 当前同步夜间任务的终态。 |
