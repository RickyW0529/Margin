...
`07-strategy_config` 是 Margin v0.1 的策略配置模块，负责定义、存储、验证、版本化以及生命周期管理投资策略。每个策略由可编辑的 `StrategyProfile`（档案）和一组不可变的 `StrategyVersion`（版本快照）组成，支持从内置模板创建、自定义创建、版本更新、沙盒验证、回测/模拟交易、激活、暂停与归档等完整生命周期。
